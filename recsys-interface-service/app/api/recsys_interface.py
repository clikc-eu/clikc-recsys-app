...
async def get_recommendations_for_user(user_id: int, is_last_lm: bool, last_lu_id: int = -1, result: float = 1.0, liked: bool = True, client_session: ClientSession = Depends(client_session_dep), api_key: APIKey = Depends(authentication)):
    ...
    return await RecsysInterfaceService().get_recommendations_for_user(user_id=user_id, is_last_lm=is_last_lm, last_lu_id=last_lu_id, result=result, liked=liked, client_session=client_session)